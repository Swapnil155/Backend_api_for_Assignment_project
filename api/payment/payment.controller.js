...
const stripe = new Stripe(process.env.SECERT_KEY)
...
module.exports = {
    payement: async (req, res) => {
        ...
        try {
            ...
            if (!makePayment) {
                ...
                const Update_user_info = await User.findByIdAndUpdate(user_id, {
                    $set: {
                        payment: makePayment,
                        plan: {
                            name: plan,
    ...
    UpdatePaymentStatus: async (req, res) => {
        ...
        try {
            const Update_Payment_info = await Payment.findByIdAndUpdate(payment_id, {
                $set: {
                    status: `succeeded`
                }
            }, { new: true })
            ...
            const user_all_Details = await User.findOne({ _id: user_id }).populate('userDetails').populate('payment')
